...
Cornipickle.CornipickleProbe = function()
{
	...
	/**
	 * List of current transformations that can be applied
	 */
...
Cornipickle.CornipickleProbe.handleResponse = function(response)
{
	...
	for (var i = 0; i < response["highlight-ids"].length; i++)
	{
		...
		for (var j = 0; j < set_of_tuples.length; j++)
		{
			...
			{
				var el_id = tuple[k];
				Cornipickle.CornipickleProbe.highlightElement(el_id, i, "red");
			}
		}
		// Show explanation
 ...
 */
Cornipickle.CornipickleProbe.highlightElement = function(id, tuple_id, color)
{
	...
	in_html += "style=\"pointer-events:none;opacity:0.1;border-radius:3px;cursor:help;position:absolute;border:4px solid " + color + ";";
